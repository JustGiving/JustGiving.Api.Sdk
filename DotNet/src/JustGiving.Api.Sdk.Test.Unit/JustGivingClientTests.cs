...
namespace GG.Api.Sdk.Test.Unit
{
    ...
    class JustGivingClientTests
    {
        ...
        public void Ctor_WhenHttpClientIsProvidedAndValid_Constructs()
        {
            ...
            new JustGivingClient(new ClientConfiguration("http://api.local.justgiving.com/", "000", 1), client);
        }

        [Test]
        public void Ctor_WhenHttpClientIsProvidedAndNull_ThrowsArgumentNullException()
        {
            var exception = Assert.Throws<ArgumentNullException>(() => new JustGivingClient(new ClientConfiguration("http://api.local.justgiving.com/", "000", 1), null));
